...
def initialize_parameters(n_f):
    w1 = tf.get_variable('w1', [80, n_f], initializer=tf.keras.initializers.he_normal())
    b1 = tf.get_variable('b1', [80, 1], initializer=tf.zeros_initializer())
    w2 = tf.get_variable('w2', [60, 80], initializer=tf.keras.initializers.he_normal())
    b2 = tf.get_variable('b2', [60, 1], initializer=tf.zeros_initializer())
    w3 = tf.get_variable('w3', [40, 60], initializer=tf.keras.initializers.he_normal())
    b3 = tf.get_variable('b3', [40, 1], initializer=tf.zeros_initializer())
    w4 = tf.get_variable('w4', [1, 40], initializer=tf.keras.initializers.he_normal())
...
def add_layer(W, b, X, activation=tf.nn.relu):
    Z = tf.matmul(W, X) + b
    A = activation(Z)
    return A
...
def compute_cost(Y, Z_out):
    Y = tf.log(tf.clip_by_value(Y, 1e-8, math.inf))
    Z_out = tf.log(tf.clip_by_value(Z_out, 1e-8, math.inf))
...
def model(train_x, train_y, valid_x, valid_y, learing_rate=0.001, num_epoch=10000, minibatch_size=128, dropout_rate=0.,
    ...
    with tf.Session() as sess:
        ...
        predict = Z_out.eval({X:valid_x, drop_rate:0.})
        for i in range(10):
...
if __name__ == '__main__':
    ...
    model(X_train, Y_train, X_valid, Y_valid, dropout_rate=0.)